...
GLint GLSLShader::GetUniformLocation(GLchar const* name, bool exit_on_error) {
...
void GLSLShader::DeleteShaderProgram() {
    if (pgm_handle > 0) {
        ...
        if (static_cast<GLint>(pgm_handle) == current)  // check if shader is currently bound
            glUseProgram(0);        // unbind it if so
...
GLboolean GLSLShader::CompileShaderFromFile(GLenum shader_type, const std::string& file_name) {
    ...
    //  return GL_FALSE;
    //}
    std::stringstream shader_file = CEO::Instance().GetManager<FileManager>()->ReadFile(file_name, false, std::ios_base::in);
    if (shader_file.str().empty()) {
        LOGE("File not found");
        log_string = "File not found";
        return GL_FALSE;
    }
    ...
    if (0 == pgm_handle) {
        LOGE("Cannot create program handle");
...
void GLSLShader::SetUniform(GLchar const* name, Color const& color) {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
...
void GLSLShader::SetUniform(const GLchar* name, const Vec2& val) {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
        glUniform2f(loc, val.x, val.y);
    } else {
...
void GLSLShader::SetUniform(const GLchar* name, const Mat3& val) {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
        glUniformMatrix3fv(loc, 1, GL_FALSE, &val.m[0]);
    } else {
        std::cout << "Uniform variable " << name << " doesn't exist" << std::endl;
    }
}
