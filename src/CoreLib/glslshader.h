...
class GLSLShader {	/*! GLSLShader class. */
	public:
		using ShaderPair = std::pair<GLenum, std::string>;	// pair of <shader type, shader file path>
		...
		GLboolean CompileLinkValidate(std::vector<ShaderPair>, bool compile_from_file = true);
		...
		void SetUniform(GLchar const* name, Color const& color);
		...
		// Added on to incorporate our Math Library
		void SetUniform(const GLchar* name, const Vec2& val);
		void SetUniform(const GLchar* name, const Mat3& val);
